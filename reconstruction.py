...
def prepare_for_fresnel ( input_matrix):
        return np.exp(1j * input_matrix * 2 * np.pi / 256)


def fresnel_transform (pixel_holo_size_h, pixel_holo_size_w, distance, wavelenght, input_matrix):
    ...
    fourier = np.fft.ifftshift(np.fft.fft2(np.fft.fftshift(input_matrix * first_fresnel_factor)))
    return fourier * second_fresnel_factor

def inverse_fresnel_transform (pixel_holo_size_h, pixel_holo_size_w, distance, wavelenght, input_matrix):
    N_h, N_w = input_matrix.shape
    scale_h = distance * wavelenght / ( N_h * pixel_holo_size_h**2)
    scale_w = distance * wavelenght / ( N_w * pixel_holo_size_w**2)
    inverse_first_fresnel_factor = np.exp(
                    np.array([
                        [
                            -1j * np.pi * ((i- int(N_h / 2))**2 /( N_h * scale_h) 
                            + (j - int(N_w / 2))**2 / (N_w * scale_w))
    ...
    inverse_second_fresnel_factor = np.exp(
                    np.array([
                        [
                            -1j * np.pi * (scale_h * (i- int(N_h / 2))**2 / N_h  
                            + scale_w * (j - int(N_w / 2))**2 / N_w)
    ...
    inv_fourier = np.fft.ifftshift(np.fft.ifft2(np.fft.ifftshift(input_matrix * inverse_first_fresnel_factor)))
    return inv_fourier * inverse_second_fresnel_factor
...
img = abs(angle_spect_transform(pixel_holo_size_h, pixel_holo_size_w, dist, wavelenght, prepare_for_fresnel(holo)))**2
